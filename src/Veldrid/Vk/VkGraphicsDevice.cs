...
namespace Veldrid.Vk
{
    internal unsafe class VkGraphicsDevice : GraphicsDevice
    {
        ...
        {
            if (mainSwapchain?.LowLatencySupported != true) return;

            // blocks until the driver decides CPU work should begin, then opens the frame.
            mainSwapchain.LatencySleep();
            // TODO: consider moving the following markers to o!f's input and update threads, respectively to better take their latency into account.
            //       These calls are thread safe. However, it's a bit unclear how reflex will behave if the markers are set multiple times per frame
            //       and in arbitrary order, so for now we leave them here and assume that the 1000hz tick rate of input/update is effectively instant.
